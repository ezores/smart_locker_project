...
class TestAPI:
    ...
    def test_borrow_item_success(self):
        ...
        items_response = self.session.get(f"{API_BASE}/items",
                                        headers=self.get_auth_headers())
        ...
        if len(items) > 0:
            ...
            if available_item:
                ...
                response = self.session.post(f"{API_BASE}/borrows",
                                          json=borrow_data,
                                          headers=self.get_auth_headers())
    ...
    def test_borrow_item_unauthorized(self):
        ...
        response = self.session.post(f"{API_BASE}/lockers/borrow",
                                   json=borrow_data)
        assert response.status_code == 401
    ...
    def test_return_item_success(self):
        ...
        if len(items) > 0:
            ...
            if available_item:
                ...
                borrow_response = self.session.post(f"{API_BASE}/borrows",
                                                 json=borrow_data,
                                                 headers=self.get_auth_headers())
                ...
                response = self.session.post(f"{API_BASE}/borrows/{borrow_id}/return",
                                          json=return_data,
                                          headers=self.get_auth_headers())
                assert response.status_code == 200
    ...
    def test_locker_operations(self):
        ...
        # Get lockers
        lockers_response = self.session.get(f"{API_BASE}/lockers",
                                          headers=self.get_auth_headers())
        ...
        if len(lockers) > 0:
            ...
            # Test open locker
            open_response = self.session.post(f"{API_BASE}/lockers/{locker_id}/open",
                                            headers=self.get_auth_headers())
            assert open_response.status_code == 200
            
            # Test close locker
            close_response = self.session.post(f"{API_BASE}/lockers/{locker_id}/close",
                                             headers=self.get_auth_headers())
            assert close_response.status_code == 200
            
            # Test get locker status
            status_response = self.session.get(f"{API_BASE}/lockers/{locker_id}/status",
                                             headers=self.get_auth_headers())
    ...
    def test_borrow_history(self):
        ...
        # Get borrows
        response = self.session.get(f"{API_BASE}/borrows",
                                  headers=self.get_auth_headers())
        assert response.status_code == 200
        data = response.json()
        assert "borrows" in data
    ...
    def test_admin_export_endpoints(self):
        ...
        # Test export borrows
        response = self.session.get(f"{API_BASE}/admin/export/borrows",
                                  headers=self.get_auth_headers())
        assert response.status_code == 200
    ...
    def test_rs485_test_endpoint(self):
        ...
        response = self.session.get(f"{API_BASE}/admin/rs485/test",
                                  headers=self.get_auth_headers())
        assert response.status_code == 200
        data = response.json()
    ...
    def test_borrow_unavailable_item(self):
        ...
        if len(items) > 0:
            ...
            if available_item:
                ...
                second_borrow_response = self.session.post(f"{API_BASE}/borrows",
                                                         json=borrow_data,
                                                         headers=self.get_auth_headers())
    ...
    def test_return_nonexistent_borrow(self):
        ...
        response = self.session.post(f"{API_BASE}/borrows/99999/return",
                                   json=return_data,
                                   headers=self.get_auth_headers())
        assert response.status_code == 404
