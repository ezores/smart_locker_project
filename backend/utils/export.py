...
class ExportManager:
    ...
    def export_excel(
        self, data: List[Dict], filename: str = None, sheet_name: str = "Data"
    ) -> bytes:
    ...
    def export_pdf(
        self, title: str, sections: List[Dict], filename: str = None
    ) -> bytes:
    ...
    def create_system_report(
        self, db_session, report_type: str = "comprehensive"
    ) -> Dict[str, Any]:
        """Create comprehensive system report"""
        from app import Borrow, Item, Locker, Log, Return, User
        ...
        if report_type == "comprehensive":
            ...
            recent_logs = [
                log.to_dict()
                for log in Log.query.order_by(Log.timestamp.desc()).limit(100).all()
            ]
    ...
    def export_system_report(
        self, db_session, format_type: str = "pdf", report_type: str = "comprehensive"
    ) -> bytes:
        ...
        if format_type == "csv":
            ...
            if report_type == "comprehensive":
                ...
                summary_data = [
                    {"metric": k, "value": v} for k, v in report_data["summary"].items()
                ]
...
def export_system_report(
    db_session, format_type: str = "pdf", report_type: str = "comprehensive"
) -> bytes:
    """Export system report in specified format"""
